...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            String sProcName = "RemoveFromGroup";
            if (args.length == 0) {
                printHelp(sProcName);
                return;
            }
            ...
            if (statusListener.waitEvents() && statusListener.isConnected()) {
                ...
                O2GRequest request = removeOrdersFromGroupRequest(session, sampleParams.getAccountID(), orderIDList);
                if (request == null) {
    ...
    private static O2GRequest removeOrdersFromGroupRequest(O2GSession session, String sAccountID, List<String> orderIDs) throws Exception {
    ...
    private static void printSampleParams(String procName,
        ...
        if (prm != null) {
            System.out.println(String.format("OrderID='%s', AccountID='%s'",
                    prm.getOrderID(), prm.getAccountID()));
        }
    }
}