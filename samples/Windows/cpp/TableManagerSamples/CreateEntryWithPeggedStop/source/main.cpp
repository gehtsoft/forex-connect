...
IO2GRequest *createEntryOrderRequestWithPeggedStop(IO2GSession *session, const char *sOfferID, const char *sAccountID, int iAmount,
    double dRate, const char *sBuySell, const char *sOrderType, const char *expDate, bool isPegged, const char* sPegType, double pegOffset, double rateStop);
...
{
    std::string procName = "CreateEntryWithPeggedStop";
    if (argc == 1)
    {
        printHelp(procName);
        return -1;
    ...
    if (bConnected)
    {
        ...
        if (account)
        {
            std::cout << "Account_sample: " << sampleParams->getAccount() << std::endl;
            std::cout << "Account_real: " << account->getAccountID() << std::endl;
            if (bIsAccountEmpty)
            {
            ...
            if (offer)
            {
                ...
                if (loginRules)
                {
                    ...
                    O2G2Ptr<IO2GRequest> request = createEntryOrderRequestWithPeggedStop(session, offer->getOfferID(), account->getAccountID(), iAmount,
                        sampleParams->getRate(), sampleParams->getBuySell(), sOrderType.c_str(), sampleParams->getExpDate(), 
                        sampleParams->IsPegged(), sampleParams->getPegType(),
...
void printSampleParams(std::string &sProcName, LoginParams *loginParams, SampleParams *sampleParams)
{
    ...
    if (sampleParams)
    {
        std::cout << "Instrument='" << sampleParams->getInstrument() << "', "
                  ...
                  << "StopRate='" << sampleParams->getRateStop() << "', "
...
void printHelp(std::string &sProcName)
{
    ...
    std::cout << "Rate of the stop order for nonpegged mode." << std::endl << std::endl;
    ...
    double dRate, const char *sBuySell, const char *sOrderType, const char *expDate, bool isPegged, const char* sPegType, double pegOffset, double rateStop)
{
    ...
    valuemap->setString(CustomID, "CreateEntryWithPeggedStop");

    if (isPegged)
    {
        valuemap->setDouble(PegOffsetStop, buyOrder ? -pegOffset : pegOffset);
        valuemap->setString(PegTypeStop, sPegType);
    }
    else
    {
        valuemap->setDouble(Rate, rateStop);
    }
