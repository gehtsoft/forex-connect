...
DATE HP_TOOLS date::DateConvertTz(DATE dt, eTimeZone tzFrom, eTimeZone tzTo)
{
    if (!g_bTZinitialized)
    {
...
bool IsDaylightTime(DATE dt, LPTIME_ZONE_INFORMATION lptzi, bool bUTC)
{
    ...
    SYSTEMTIME st;
    hptools::date::OleTimeToWindowsTime(dt, &st);
...
DATE GetDayFromTimeZoneInfo(SYSTEMTIME *lpTimeZone, int nYear)
{
    ...
    if (iWeekOrder == 5)
    {
        ...
        hptools::date::WindowsTimeToOleTime(&st, &dt);
...
char* date::DateStringToCTime(const char *buf, const char *fmt, struct tm *tm)
{
    char c;
    const char *bp;
    size_t len = 0;
