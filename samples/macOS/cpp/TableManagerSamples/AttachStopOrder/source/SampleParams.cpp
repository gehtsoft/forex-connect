...
const char *SampleParams::Strings::pegtypeNotSpecified = "'Pegtype' is not specified (/pegtype | --pegtype)";
const char *SampleParams::Strings::pegoffsetNotSpecified = "'Pegoffset' is not specified (/pegoffset | --pegoffset)";
...
SampleParams::SampleParams(int argc, char **argv)
{
    ...
    mIsPegged = isKeyExist(argc, argv, "ispegged");
    mPegType = getArgument(argc, argv, "pegtype");
    ...
    std::string sPegOffset = getArgument(argc, argv, "pegoffset");
    ...
    if (sPegOffset.empty())
        ...
        mPegOffset = atof(sPegOffset.c_str());
...
bool SampleParams::isKeyExist(int argc, char **argv, const char *key)
{
...
bool SampleParams::IsPegged()
{
    return mIsPegged;
}

const char *SampleParams::getPegType()
{
    return mPegType.c_str();
}

double SampleParams::getPegOffset()
{
    return mPegOffset;
}
