...
class LoginViewController : UIViewController, UITextFieldDelegate {
    ...
    @IBAction func connectBtnClicked(sender: UIButton) {
        ...
        loginJob.async {
            self.forexConnect.setLoginData(user: userNameValue, pwd: passwordValue, url: host+suffix, connection: connectionValue, sessionId: sessionIdValue)
    ...
    func completeLoginJob() {
        ...
        mainQueue.async {
            ...
            // if login have no accounts then logout
            if self.forexConnect.allAccountsId.count == 0 {
                self.forexConnect.logout()
                self.showErrorAlert()
                return
            }
