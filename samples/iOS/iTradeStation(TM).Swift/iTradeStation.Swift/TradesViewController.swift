...
class TradesTableRow
{
    public let tradesId: Int64
    public let instrument: String
    public let amount: String
    public let buySell: String
    public let pl: Double
    
    init(tradesId: Int64, instrument: String, buySell: String, amount: String, pl: Double) {
...
class TradesTable : IO2GTableListener, IO2GEachRowListener
{
    ...
    var tradesUpdateNotificator = { ([TradesTableRow]) -> () in  }
    var tradesTableRows = [String:TradesTableRow]()
    
    init() {
        let tableManager = ForexConnect.sharedInstance.getTableManager()
    ...
    func notify() {
        ...
        tradesUpdateNotificator(sorderArray)
    ...
    private func addTrade(row: IO2GTradeTableRow) {
        ...
        let pl = row.getPL()
        let tradesId = (row.getTradeID() as NSString).longLongValue
    
        let tableRow: TradesTableRow = TradesTableRow(tradesId: tradesId, instrument: instrument!, buySell: buySell!, amount: amount, pl: pl)
...
class TradesViewController : UITableViewController
{
    ...
    override func viewDidLoad() {
        ...
        headerLabel.text = "    Symbol     Amount       S/B       P/L "
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let amount = tradesTableRows[indexPath.row].amount
        let pl = tradesTableRows[indexPath.row].pl
        ...
        plLable.text = pl.parseToPlaces(places: 1)
    ...
    func tradesUpdated(tradesTableRows: [TradesTableRow]) {
        ...
        DispatchQueue.main.async {
            // calc total PL
            var summ = 0.0
            for var tradeRow in tradesTableRows {
                summ += tradeRow.pl
            }
            // add total row
            let totalRow = TradesTableRow(tradesId: 0, instrument: "Total", buySell: "", amount: "", pl: summ)
