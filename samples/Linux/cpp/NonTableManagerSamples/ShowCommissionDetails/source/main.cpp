...
void printCommissionDetails(IO2GSession *session, SampleParams *sampleParams);
const char* getO2GCommissionStageDescr(O2GCommissionStage stage);
const char* getO2GCommissionUnitTypeDescr(O2GCommissionUnitType ut);

int main(int argc, char *argv[])
{
    std::string procName = "ShowCommissionDetails";
    if (argc == 1)
    {
        printHelp(procName);
        return -1;
...
{    
    ...
    O2G2Ptr<IO2GCommissionDescriptionsCollection> commList = commissionProvider->getCommissionDescriptions(offer->getOfferID(), account->getATPID());
    for (int i = 0; i < commList->size(); i++)
    {
        O2G2Ptr<IO2GCommissionDescription> descr = commList->get(i);
        std::cout << "\nCommission " << i + 1 << ": "<< getO2GCommissionStageDescr(descr->getStage()) << ", " << getO2GCommissionUnitTypeDescr(descr->getUnitType()) << ", " << descr->getCommissionValue();
    }
...
const char* getO2GCommissionStageDescr(O2GCommissionStage stage)
{
    switch (stage)
    {
    case CommissionStageUnknown:
        return "CommissionStageUnknown";
    case OpenCommission:
        return "OpenCommission";
    case CloseCommission:
        return "CloseCommission";
    case AnyDealCommission:
        return "AnyDealCommission";
    }
...
const char* getO2GCommissionUnitTypeDescr(O2GCommissionUnitType ut)
{
    switch (ut)
    {
    case CommissionTypeUnknown:
        return "CommissionTypeUnknown";
    case CommissionPerLot:
        return "CommissionPerLot";
    case CommissionPerLotConv:
        return "CommissionPerLotConv";
    case CommissionPerTrade:
        return "CommissionPerTrade";
    case CommissionPerOrder:
        return "CommissionPerOrder";
    case CommissionPerOrderConv:
        return "CommissionPerOrderConv";
    case CommissionPerBasisPoints:
        return "CommissionPerBasisPoints";
    }
