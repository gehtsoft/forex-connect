...
namespace RemoveFromGroup
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                Console.WriteLine("RemoveFromGroup sample\n");

                ArgumentParser argParser = new ArgumentParser(args, "RemoveFromGroup");
                ...
                if (statusListener.WaitEvents() && statusListener.Connected)
                {
                    ...
                    O2GRequest request = RemoveOrderFromGroupRequest(session, sampleParams.AccountID, sampleParams.OrderID);
                    if (request == null)
                    {
