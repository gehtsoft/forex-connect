...
public class ResponseListener implements IO2GResponseListener {
    ...
    public ResponseListener(O2GSession session) {
        mSession = session;
        mRequestID = "";
        mOrderIDs = new ArrayList<String>();
        mResponse = null;
            ...
            for (int ii = 0; ii < reader.size(); ii++) {
                if (reader.getUpdateTable(ii) == O2GTableType.ORDERS) {
                    ...
                    if (reader.getUpdateType(ii) == O2GTableUpdateType.UPDATE) {
                        if (mOrderIDs.contains(orderRow.getOrderID())) {
                            System.out.println(String.format("The order has been updated. OrderID=%s, ContingentOrderID=%s, ContingencyType=%s",
                                    orderRow.getOrderID(), orderRow.getContingentOrderID(), orderRow.getContingencyType()));
                            mOrderIDs.remove(orderRow.getOrderID());
                        }
                        if (mOrderIDs.size() == 0) {
                            mSemaphore.release();
                        }
