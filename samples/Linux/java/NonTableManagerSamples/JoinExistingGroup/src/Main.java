...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            String sProcName = "JoinExistingGroup";
            if (args.length == 0) {
                printHelp(sProcName);
                return;
            }
            ...
            if (statusListener.waitEvents() && statusListener.isConnected()) {
                ...
                orderIDList.add(sampleParams.getOrderID());
                ...
                if (!isContingencyExists(session, sampleParams.getAccountID(), sampleParams.getContingencyID(), responseListener)) {
                    throw new Exception(String.format("Contingency group '%s' does not exist", sampleParams.getContingencyID()));
                }

                O2GRequest request = addOrdersIntoGroupRequest(session, sampleParams.getAccountID(), sampleParams.getContingencyID(), orderIDList, iContingencyGroupType);
                if (request == null) {
    ...
    private static boolean isContingencyExists(O2GSession session, String sAccountID, String sContingencyID, ResponseListener responseListener) throws Exception {
        boolean bHasOrder = false;
        O2GRequestFactory requestFactory = session.getRequestFactory();
        if (requestFactory == null) {
            throw new Exception("Cannot create request factory");
        ...
        for (int i = 0; i < responseReader.size(); i++) {
            O2GOrderRow orderRow = responseReader.getRow(i);
            if (sContingencyID.equals(orderRow.getContingentOrderID())) {
                bHasOrder = true;
                break;
            }
        }
    ...
    private static O2GRequest addOrdersIntoGroupRequest(O2GSession session, String sAccountID, String sContingencyID, List<String> orderIDs, int iContingencyType) throws Exception {
        ...
        valuemapMain.setString(O2GRequestParamsEnum.CONTINGENCY_ID, sContingencyID);

        for (String sOrderID : orderIDs) {
            O2GValueMap valuemapChild = requestFactory.createValueMap();
            valuemapChild.setString(O2GRequestParamsEnum.ORDER_ID, sOrderID);
    ...
    private static void printHelp(String sProcName)
    {
        ...
        System.out.println("Order, which you want to remove from existing contingency group.\n");
        
        System.out.println("/contingencyid | --contingencyid ");
        System.out.println("Contingency ID of group, to which you want to join an entry order. Mandatory argument.\n");
    ...
    private static void checkObligatoryParams(LoginParams loginParams, SampleParams sampleParams) throws Exception {
        ...
        if(sampleParams.getContingencyID().isEmpty()) {
            throw new Exception(SampleParams.CONTINGENCYID_NOT_SPECIFIED);
        }
    ...
    private static void printSampleParams(String procName,
        ...
        if (prm != null) {
            System.out.println(String.format("ContingencyID='%s', OrderID='%s', AccountID='%s'",
                    prm.getContingencyID(), prm.getOrderID(), prm.getAccountID()));
        }
    }
}