...
public class SessionStatusListener implements IO2GSessionStatus {
    ...
    // ctor
    public SessionStatusListener(O2GSession session, String sSessionID, String sPin, String sSessionString) {
        mSession = session;
        mSessionID = sSessionID;
        mPin = sPin;
        mSessionString = sSessionString;
        reset();
    ...
    public void onSessionStatusChanged(O2GSessionStatusCode status) {
        System.out.println(mSessionString + ": " + status.toString());
        switch (status) {
