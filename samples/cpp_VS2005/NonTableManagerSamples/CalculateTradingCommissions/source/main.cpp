...
void printEstimatedTradingCommissions(IO2GSession *session, SampleParams *sampleParams);
...
{    
    ...
    std::cout << "Commission for open the position is " << commissionProvider->calcOpenCommission(offer, account, amount, sampleParams->getBuySell(), 0) << std::endl;
    std::cout << "Commission for close the position is " << commissionProvider->calcCloseCommission(offer, account, amount, sampleParams->getBuySell(), 0) << std::endl;
    std::cout << "Total commission for open and close the position is " << commissionProvider->calcTotalCommission(offer, account, amount, sampleParams->getBuySell(), 0, 0) << std::endl;
}
